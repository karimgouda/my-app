...
<body>
        ...
        <div class="container-fluid">
          ...
          <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                ...
                @if($user->role =="admin")
                    <li class="nav-item">
                        <a class="nav-link active" aria-current="page" href="{{ url('/createBook') }}">Add new Book</a>
                    </li>
                    @endif
                <li class="nav-item">
    ...
    <div class=" w-50 m-auto  all text-white mt-5">
        <h1 class="text-center text-white -50 mb-3">All Books</h1>
        @foreach ($books as $book )


    <h4 class=" text-center bg-primary my-2">{{$loop->iteration}} - <a href="{{url("/showBook/$book->id")}}"> {{ $book->title }}</a></h4>
    @endforeach
    </div>

    <div class=" w-25 m-auto mt-5">
      <div class=" w-25 m-auto">{{$books->links()}}</div>
    </div>
